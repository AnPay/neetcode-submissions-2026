...
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        ...
        //total+target should be divisible by 2
        ...
        for(int n :nums)
        {
            //Reverse loop = don’t reuse the same element in the same iteration
            for(int j=s;j>=n;j--)
            {
                dp[j]+=dp[j-n];
            }
...
Pattern: Subset problem
State:dp[i] represent number of ways to make sum i
Transition:
for(int n :nums)
    for(int j =P;j>=n;j++)
        dp[j]+=dp[j-n]
Edge cases: target>total, all -ve numbers
Complexity:o(n*P)
*/
